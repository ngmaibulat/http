...
```bash
npm test          # runs straight off src/*.ts via Node's type stripping; no build, no network
npm run build     # compile to dist/ and mark the bins executable
npm run test:dist # the same suite against the built artifact
```
